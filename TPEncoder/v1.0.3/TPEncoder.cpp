...
void __encode(TPEncoderConfig const& conf) {
	...
	while(row < conf.picHeight) {
		RGBs[(col + (conf.picHeight - row - 1) * conf.picWidth) * 3 + 0] = conf.dB + (rand() & (conf.sB - 1));
		RGBs[(col + (conf.picHeight - row - 1) * conf.picWidth) * 3 + 1] = conf.dG + (rand() & (conf.sG - 1));
		RGBs[(col + (conf.picHeight - row - 1) * conf.picWidth) * 3 + 2] = conf.dR + (rand() & (conf.sR - 1));
		++col;
		if(col == conf.picWidth) {
			++row;
			col = 0;
